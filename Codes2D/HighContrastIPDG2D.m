function [OP,MM] = HighContrastIPDG2D(a)

% Purpose: Set up the discrete matrix for the High Contrast problem
%          directly using LDG. The operator is set up in the weak form.
%          The function a is assumed to be constant in each triangle.  
Globals2D;
...
for k1=1:K 
  ...
  OP11 = a1* J(1,k1)*(Dx'*MassMatrix*Dx + Dy'*MassMatrix*Dy);
  ...
  for f1=1:Nfaces
    ...
    switch(BCType(k1,f1))
      case {Dirichlet}
	OP11 = OP11 + ( a1* gtau*mmE - a1* mmE*Dn1 - a1* Dn1'*mmE ); % ok
      case {Neuman}
        % nada 
      otherwise
